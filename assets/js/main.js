...
const app = {
    ...
    handleDom: function () {
        ...
        //Xử lý volume khi click vào volumemute để quay lại âm lượng trước khi mute
        var crVl = [volume.value];
        //Xử lý thanh volume
        volume_change = (current) => {
            ...
            crVl.push(volume.value);
        };
        unmute_sound = (current) => {
            audio.muted = false;
            volume.value = crVl[crVl.length - 1];
            audio.volume = volume.value / 100;
        };

        volumeCurrent.onclick = function () {
            volumeCurrent.classList.toggle("mute");
        };
    },
